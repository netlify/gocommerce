...
func NewAPI(config *conf.Configuration, db *gorm.DB, mailer *mailer.Mailer) *API {
	...
	mux.Get("/users/", api.trace(api.GetAllUsers))
	mux.Get("/users/:user_id", api.trace(api.GetSingleUser))
	mux.Get("/users/:user_id/addresses", api.trace(api.GetAllAddresses))
	mux.Get("/users/:user_id/addresses/:addr_id", api.trace(api.GetSingleAddress))
