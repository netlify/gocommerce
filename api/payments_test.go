...
func TestPaymentsRefund(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		NewAPIWithVersion(ctx, test.GlobalConfig, logrus.StandardLogger(), test.DB, nil, defaultVersion).handler.ServeHTTP(w, r)
...
func TestPaymentPreauthorize(t *testing.T) {
	t.Run("PayPal", func(t *testing.T) {
		...
		t.Run("Form", func(t *testing.T) {
			...
			ctx, err := WithInstanceConfig(context.Background(), globalConfig.SMTP, test.Config, "")
			require.NoError(t, err)
			NewAPIWithVersion(ctx, test.GlobalConfig, logrus.StandardLogger(), test.DB, nil, "").handler.ServeHTTP(recorder, req)
